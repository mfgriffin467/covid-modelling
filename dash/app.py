...
df_filt1 = df_final[['date','country','max_pop','beta','projected_susceptible','projected_infections', 'projected_recovered', 'projected_hospitalisation', 'projected_icu', 'projected_beds']].melt(id_vars = ['date', 'country','max_pop','beta'], var_name = 'projection')
df_filt2 = df_final[['date','country','max_pop','beta', 'projected_hospitalisation', 'projected_icu', 'projected_beds']].melt(id_vars = ['date', 'country','max_pop','beta'], var_name = 'projection')
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Dropdown(
		    id='ctry',
            options=[{'label': i, 'value': i} for i in country_list['country'].unique()],
            value='China'
            ),
        html.Br(),
...
#    html.H2(children='Covid cases'),

    html.Div([
        dcc.Graph(id='graphic1')
        ]),

#    html.H2(children='Covid deaths'),

    html.Div([
        dcc.Graph(id='graphic2')
        ]),

#    html.H2(children='Fatality rate = deaths/cases'),
    ...
    html.Div([
        ...
        dcc.Slider(
            ...
            step=0.025,
            value=0.2,
            marks={i: '{}'.format(i) for i in df_final['beta'].unique()}
            ),
        ]),
...
def update_graph(ctry,beta_select,max_pop):
    return {
            'data': [dict(
                x=df_final[(df_final.country == ctry) & (df_final.beta.between(beta_select-0.001,beta_select+0.001)) & (df_final.max_pop.between(max_pop-0.001,max_pop+0.001))]['date'],
                y=df_final[(df_final.country == ctry) & (df_final.beta.between(beta_select-0.001,beta_select+0.001)) & (df_final.max_pop.between(max_pop-0.001,max_pop+0.001))]['projected_infections'],
                mode='lines+markers',
                opacity=0.7,
                marker={
                    'size': 10,
